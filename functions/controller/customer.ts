...
export const calculateDelivery = async (req: Request, res: Response) => {
  try {
    ...
    if (axios.isAxiosError(error)) {
      return res
        .status(400)
        .send({ error: "ERR: Failed to calculate delivery fee" });
    }

    res
      .status(400)
      .send({
        error: (error as Error).message ?? "ERR: Failed to set address",
      });
  }
};
...
export const updateAptAndBusiness = async (req: Request, res: Response) => {
  try {
      ...
      .set(
        {
          address: {
            ...
            business: isEmpty(req.body.business)
              ? address.business ?? ""
              : req.body.business,
          },
        } as ICustomer,
    ...
    res
      ...
      .send({
        error:
          (error as Error).message ?? "Err: Failed to update apt or business",
      });
  }
};
...
export const removeAptAndBusiness = async (req: Request, res: Response) => {
  try {
    ...
    res
      ...
      .send({
        error:
          (error as Error).message ?? "Err: Failed to remove apt and business",
      });
  }
};
...
export const getOrderHistory = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(400)
      .send({
        error: (error as Error).message ?? "ERR: Failed to get order history",
      });
  }
};
