...
import {
  createPaymentIntent,
  generatePublicPaymentList,
  getCustomerId,
  handleConfirmingOrder,
  retrieveIntentFromCookie,
} from "../utils/payment";
...
export const getSavedPaymentMethodList = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    res
      .status(400)
      .send({
        error: (error as Error).message ?? "Failed to get payment list",
      });
  }
};
...
export const updatePaymentIntent = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(400)
      .send({ error: (error as Error).message ?? "Failed to update intent" });
  }
};

// pay with existing payment method
export const payWithSavedPaymentMethod = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    let customer = (
      await firestore().collection("usersTest").doc(req.user.uid).get()
    ).data() as ICustomer;
    ...
    res
      .status(400)
      .send({
        error: (error as Error).message ?? "Failed to process payment with id",
      });
  }
};
...
export const payWithPaymentIntent = async (req: Request, res: Response) => {
  try {
    ...
    if (isEmpty(s_id)) {
      throw new Error(
        "Unable to find the s_id, please refresh the payment page"
      );
    }
    ...
    let { customer } = await handleConfirmingOrder(
      cart,
      req.user.uid,
      stripe_result
    );
