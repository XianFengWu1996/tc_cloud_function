...
export const verifyCode = async (req: Request, res: Response) => {
  try {
    ...
    let code_data = (
      await firestore().collection("/sms_verification").doc(c_id).get()
    ).data() as ICodeData;
    ...
    res
      .status(400)
      .send({
        error: (error as Error).message ?? "ERR: Failed to verify sms code",
      });
  }
};
    ...
    res
      ...
      .send({
        error:
          ...
          "ERR: Failed to verify send message, please try again",
      });
  }
};
