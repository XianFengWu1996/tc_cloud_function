...
const handleRewardPointCalculation = (_: IHandleRewardPointCalculation) => {
  ...
  let reward = Math.round(
    _.cart.subtotal * Number(process.env.REWARD_PERCENTAGE)
  );

  // the new point will be the old points minus the amount redeem plus the reward for this order
  let updated_reward_point =
    _.customer.reward.points - _.cart.point_redemption + reward; // original point minus the redeem amount plus the new reward amount
...
export const handlePlaceInstoreOrder = async ({ user, cart }: IPlaceOrder) => {
  ...
  await firestore().runTransaction(async (transaction) => {
    ...
    let { updated_reward_point, updated_reward_transactions, reward_earned } =
      handleRewardPointCalculation({
        cart,
        customer,
      });
...
export const handlePlaceOnlineOrder = async ({
  user,
  cart,
  payment_intent_id,
}: IPlaceOrder) => {
...
export const handleConfirmingOrder = async (
  cart: ICart,
  user_id: string,
  stripe_result: Stripe.Response<Stripe.PaymentIntent>
) => {
  let customer = {} as ICustomer;
  ...
  await firestore().runTransaction(async (transaction) => {
    ...
    const { updated_reward_point, updated_reward_transactions, reward_earned } =
      handleRewardPointCalculation({ cart, customer });
      ...
      order_ref,
      {
        payment: {
          stripe: {
            ...
            payment_method_type:
              stripe_result.charges.data[0].payment_method_details?.type,
            card: stripe_result.charges.data[0].payment_method_details?.card
              ? {
                  brand:
                    stripe_result.charges.data[0].payment_method_details?.card
                      .brand,
                  exp_month:
                    stripe_result.charges.data[0].payment_method_details?.card
                      .exp_month,
...
export const generatePublicPaymentList = async (customer_id: string) => {
  ...
  paymentMethods.data.map((val) => {
    if (!val.card) {
      throw new Error(
        "Failed to list payment, some card information not found"
      );
    }
...
export const checkStoreOperatingStatus = (data: IStore) => {
  ...
  let special_hour = data.hours.special_hour.find((hour) => {
    return (
      hour.date.day === luxon_date.day &&
      hour.date.month === luxon_date.month &&
      hour.date.year === luxon_date.year
    );
  });
  ...
  if (special_hour) {
    ...
    if (!special_hour.open_for_business) {
      throw new Error(
        `The store will close entire day on ${special_hour.date.month}/${special_hour.date.day}/${special_hour.date.year}`
      );
    }

    // check if the store close early
    if (
      currentMinute <= special_hour.open_hour ||
      currentMinute >= special_hour.close_hour
    ) {
      throw new Error(
        `The special operating hours for today are ${convert_minute_to_format_time(
          special_hour.open_hour
        )} - ${convert_minute_to_format_time(special_hour.close_hour)}`
      );
    }
  }

  // third, check if within store operating hours
  let regular_hour = data.hours.regular_hour.find(
    (hour) =>
      hour.day_of_week.toLowerCase() === luxon_date.weekdayLong.toLowerCase()
  );
  if (regular_hour) {
    // check if the store is open on the day of the week
    if (!regular_hour.open_for_business) {
      throw new Error(
        `The store is currently close on ${regular_hour.day_of_week}`
      );
    }

    // if the current minute is not within the range, throw error
    if (
      currentMinute <= regular_hour.open_hour ||
      currentMinute >= regular_hour.close_hour
    ) {
      throw new Error(
        `The operating hours are ${convert_minute_to_format_time(
          regular_hour.open_hour
        )} - ${convert_minute_to_format_time(regular_hour.close_hour)}`
      );
    }
  }
};
